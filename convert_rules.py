...
def process_single_task(item):
    """
    独立封装的单任务处理函数，专供线程池调用。
    返回一个列表，包含下载失败的 URL（如果全部成功则返回空列表）。
    """
    ...
    for u in urls:
        ...
        if not content:
            ...
            break # 容灾机制：单个任务中任何一环断裂，立刻停止合并该任务
        combined_contents.append(content)
        
    if task_failed_urls:
        print(f"  ⚠️ [任务中止] {name} 部分源失效，为防破坏旧文件跳过处理！")
    ...
    if is_mihomo_updated or not os.path.exists(mrs_path):
        ...
        print(f"  ⏭️ [跳过编译] 原文件无修改: {os.path.basename(mrs_path)}")
...
def main():
    ...
    print(f"🚀 脚本启动：共加载 {len(source_list)} 个任务，准备启动多线程狂飙模式...")
