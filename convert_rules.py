...
def fetch_content(url, logs, tag="📥 [下载源文件]"):
    """带超时保护与反爬伪装的网络下载器 (支持自定义日志标签，区分主源与黑名单)"""
    logs.append(f"  {tag} {url}")
    try:
        ...
        err_type = "下载失败" if "下载源" in tag else "黑名单下载失败"
        logs.append(f"  ❌ [{err_type}] {url} -> {e}")
        return None
...
def process_single_task(item):
    ...
    # 🎯 提取黑名单 (支持直接写规则，也支持写 URL，混合形态支持！)
    excludes_raw = item.get('exclude', []) 
    if isinstance(excludes_raw, str): excludes_raw = [excludes_raw]
    
    logs.append(f"\n🚀 [启动任务] {name} | 类型: {rtype} | 主源: {len(urls)} 个 | 黑名单配置: {len(excludes_raw)} 项")
    ...
    # ---------------- 💡 1. 强制按 YAML 顺位下载主源 ----------------
    for u in urls:
        content = fetch_content(u, logs, tag="📥 [下载源文件]")
        if not content:
    ...
    if task_failed_urls:
        logs.append(f"  ⚠️ [任务中止] {name} 主源失效，停止合成！")
    ...
    for ex in excludes_raw:
        ex_str = str(ex).strip()
        # 智能判定：如果是链接，去云端拉取；如果是域名/IP，直接作为规则录入
        if ex_str.startswith('http://') or ex_str.startswith('https://'):
            ex_content = fetch_content(ex_str, logs, tag="🛡️ [获取黑名单源]")
            if not ex_content:
                task_failed_urls.append(ex_str)
                break # 🔒 安全第一：黑名单拿不到就直接熔断，防止发生大规模误杀
            for line in ex_content.splitlines():
                cleaned = clean_line(line)
                if cleaned and not cleaned.lower().startswith(('payload:', 'version:', 'address', '#')):
                    final_excludes.append(cleaned)
        else:
            final_excludes.append(ex_str) # 兼容直接写的纯文本黑名单规则 (如 apple.com)

    if task_failed_urls:
        logs.append(f"  ⚠️ [任务中止] {name} 黑名单源下载失败，为防误杀已物理熔断当前任务！")
        return task_failed_urls, '\n'.join(logs)
    
    # 将下载的脏数据和清洗完毕的黑名单大军，移交中央主炮火控系统
    res = process_rules(combined_contents, rtype, policy, is_strict, final_excludes)
