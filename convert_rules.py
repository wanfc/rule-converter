...
def compile_to_mrs(txt_path, mrs_path, rule_type):
    ...
    try:
        ...
        # 💡 [核心修改] 拦截因为全是 ASN 导致的空规则报错，防止污染日志
        if "empty rule" in e.stderr:
            print("     ⚠️ [中断] 规则集不支持编译 (可能全为 ASN 或空)，已跳过。")
        else:
            print(f"     详细错误: {e.stderr.strip().splitlines()[0]}")
...
def main():
    ...
    for item in source_list:
        ...
        if not valid_mrs_lines:
            print(f"  ⏭️ [跳过编译] 该规则集全为 ASN，Mihomo 底层不支持，保留 text 格式即可。")
        else:
            # MRS 二进制云端编译流程
