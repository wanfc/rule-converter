...
# 所有基础函数不再直接 print，而是追加到局部 logs 列表中
...
def process_rules(contents_list, target_type, policy_name, strict_mode):
    ...
    classical = []
    mihomo_text = []
    # 使用 OrderedDict 替代普通 {}，彻底无视环境差异，100% 锁定读取顺序
...
def save_text(path, lines, logs):
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    logs.append(f"  💾 [保存成功] {os.path.basename(path)} (精简去重后共 {len(lines)} 条)")
    return True # 表示已更新
...
def compile_to_mrs(txt_path, mrs_path, rule_type, logs):
    ...
    try:
        ...
        # 💡 [新增] 如果是因为全是 ASN 导致的空规则报错，拦截那堆难看的 Golang 崩溃代码
        if "empty rule" in e.stderr:
            logs.append("     ⚠️ [中断] 规则集不支持编译 (可能全为 ASN 或空)，已跳过。")
        else:
            logs.append(f"     详细错误: {e.stderr.strip().splitlines()[0]}") # 只打印第一行核心报错
...
def process_single_task(item):
    ...
    if task_failed_urls:
        logs.append(f"  ⚠️ [任务中止] {name} 部分源失效，跳过处理！")
    ...
    if not valid_mrs_lines:
        ...
        if is_mihomo_updated or not os.path.exists(mrs_path):
            ...
            logs.append(f"  ⏭️ [跳过编译] 原文件无修改: {os.path.basename(mrs_path)}")
...
def main():
    ...
    print(f"🚀 脚本启动：共加载 {len(source_list)} 个任务，多线程原子日志模式就绪...")
    
    all_failed_urls = []

    # 🚀 核心重构：启动线程池
    # max_workers=4：GitHub 虚拟机是 2核，开 4 个线程能完美压榨 IO 等待和 CPU 计算的重叠时间
